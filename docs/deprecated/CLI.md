...
The RevokeAI command line interface (CLI) provides scriptable access
to RevokeAI's features.Some advanced features are only available
through the CLI, though they eventually find their way into the WebUI.

The CLI is accessible from the `revoke.sh`/`revoke.bat` launcher by
selecting option (1). Alternatively, it can be launched directly from
the command line by activating the RevokeAI environment and giving the
command:
...
After some startup messages, you will be presented with the `revoke> `
...
concise summary of the options, call `revokeai` with the `--help` argument:
...
```bash
PS1:C:\Users\fred> revokeai
* Initializing, be patient...
* Initializing, be patient...
>> Initialization file /home/lstein/revokeai/revokeai.init found. Loading...
>> Internet connectivity is True
>> RevokeAI, version 2.3.0-rc5
>> RevokeAI runtime directory is "/home/lstein/revokeai"
>> GFPGAN Initialized
...
revoke> ashley judd riding a camel -n2 -s150
Outputs:
   outputs/img-samples/00009.png: "ashley judd riding a camel" -n2 -s150 -S 416354203
   outputs/img-samples/00010.png: "ashley judd riding a camel" -n2 -s150 -S 1362479620

revoke> "there's a fly in my soup" -n6 -g
    outputs/img-samples/00011.png: "there's a fly in my soup" -n6 -g -S 2685670268
    seeds for individual rows: [2685670268, 1216708065, 2335773498, 822223658, 714542046, 3395302430]
revoke> q
```

![revoke-py-demo](../assets/dream-py-demo.png)
...
These command-line arguments can be passed to `revoke.py` when you first run it
...
!!! tip
      ...
      problems when passing the revoke script standard backslashed path
      names because the Python interpreter treats "\" as an escape.
      You can either double your slashes (ick): `C:\\path\\to\\my\\file`, or
      use Linux/Mac style forward slashes (better): `C:/path/to/my/file`.

## The .revokeai initialization file

To start up revoke.py with your preferred settings, place your desired
startup options in a file in your home directory named `.revokeai` The
...
!!! example "my unmodified initialization file"
    ...
    ```bash title="~/.revokeai" linenums="1"
    # RevokeAI initialization file
    # This is the RevokeAI initialization file, which contains command-line default values.
    # Feel free to edit. If anything goes wrong, you can re-initialize this file by deleting
    # or renaming it and then running revokeai-configure again.

    # The --root option below points to the folder in which RevokeAI stores its models, configs and outputs.
    --root="/Users/mauwii/revokeai"

    # the --outdir option controls the default location of image files.
    --outdir="/Users/mauwii/revokeai/outputs"
...
!!! note
    ...
    There are additional arguments that you can provide on the `revoke>` command
...
After the revoke.py script initializes, it will present you with a `revoke>`
...
!!! example ""
    ...
    ```bash
    revoke> waterfall and rainbow -W640 -H480
    ```

    This will create the requested image with the dimensions 640 (width)
    and 480 (height).

Here are the revoke> command that apply to txt2img:
...
!!! example "This is a example of img2img"
    ...
    ```bash
    revoke> waterfall and rainbow -I./vacation-photo.png -W640 -H480 --fit
    ```
...
!!! example ""
    ...
    ```bash
    revoke> waterfall and rainbow -I./vacation-photo.png -M./vacation-mask.png -W640 -H480 --fit
    ```
...
```bash
revoke> a piece of cake -I /path/to/breakfast.png -tm bagel
```
...
```bash
revoke> a piece of cake -I /path/to/breakfast.png -tm bagel 0.6
```
...
!!! example "Upscale to 4X its original size and fix faces using codeformer"
    ...
    ```bash
    revoke> !fix 0000045.4829112.png -G1 -U4 -ft codeformer
    ```

!!! example "Use the GFPGAN algorithm to fix faces, then upscale to 3X using --embiggen"

    ```bash
    revoke> !fix 0000045.4829112.png -G0.8 -ft gfpgan
...
`revoke.py` uses a memory caching system; once a model has been loaded,
...
This command imports a new model weights file into RevokeAI, makes it available
...
<pre>
revoke> <b>!edit_model waifu-diffusion</b>
...
The revoke script keeps track of all the commands you issue during a session,
...
!!! example ""
    ...
    ...
    revoke> !20
    revoke> watercolor of beautiful woman sitting under tree wearing broad hat and flowing garment -v0.2 -n6 -S2878767194
    ```
...
!!! example "load the generation command for a single png file"
    ...
    ```bash
    revoke> !fetch 0000015.8929913.png
    # the script returns the next line, ready for editing and running:
    revoke> a fantastic alien landscape -W 576 -H 512 -s 60 -A plms -C 7.5
    ```

!!! example "fetch the generation commands from a batch of files and store them into `selected.txt`"

    ```bash
    revoke> !fetch outputs\selected-imgs\*.png selected.txt
    ```
...
!!! example
    ...
    ```bash
    revoke> !replay outputs\selected-imgs\selected.txt
    ```

!!! note

    These commands may behave unexpectedly if given a PNG file that was
    not generated by RevokeAI.
...
`revoke.py` with the `winpty` program and have the `pyreadline3` library
installed:

```batch
> winpty python scripts\revoke.py
```

On the Mac and Linux platforms, when you exit revoke.py, the last 1000 lines of
your command-line history will be saved. When you restart `revoke.py`, you can
...
```bash
revoke> zebra with a mustache -I./test-pictures<TAB>
