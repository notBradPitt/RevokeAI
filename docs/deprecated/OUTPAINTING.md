...
RevokeAI supports two versions of outpainting, one called "outpaint" and the
...
regions outside of the original image. To outpaint using the `revoke.py` command
...
```bash
revoke> a stream by a river -I /path/to/transparent_img.png
```
...
RevokeAI. Note that it works with arbitrary PNG photographs, but not currently
...
```bash
revoke> !fix images/curly.png --outcrop top 128 right 64 bottom 64
```
...
#### Outcropping non-RevokeAI images

You can outcrop an arbitrary image that was not generated by RevokeAI,
...
```bash
revoke> !fix images/vacation.png --outcrop top 128 --new_prompt "family vacation"
```
...
3. Your results will be _much_ better if you use the `inpaint-1.5` model
   released by runwayML and installed by default by `revokeai-configure`.
...
```bash
revoke> !fix images/curly.png --out_direction top 64
```
