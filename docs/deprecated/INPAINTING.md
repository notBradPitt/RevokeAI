...
```bash
revoke> "man with cat on shoulder" -I./images/man.png -M./images/man-transparent.png
```
...
algorithm. This works on any image, not just ones generated by RevokeAI.
...
```bash
revoke> a baseball -I /path/to/still_life.png -tm orange
```
...
```bash
revoke> a baseball -I /path/to/breakfast.png -tm orange 0.6
```
...
The `.masked.png` file can then be directly passed to the `revoke>` prompt in
...
```bash
revoke> !mask ./test-pictures/curly.png -tm hair 0.5
...
```bash
revoke> medusa with cobras -I ./test-pictures/curly.png -M 000019.curly.hair.masked.png -C20
...
```bash
revoke> medusa with cobras -I ./test-pictures/curly.png -tm hair -C20
```
...
You may use either the CLI (`revoke.py` script) or directly edit the
...
To use the custom inpainting model, launch `revoke.py` with the argument
...
7. After following the inpainting instructions above (either through the CLI or
   the Web UI), marvel at your newfound ability to selectively revoke. Lookin'
   good!
