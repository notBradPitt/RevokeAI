...
Place the training images in a directory on the machine RevokeAI runs
on. We recommend placing them in a subdirectory of the
`text-inversion-training-data` folder located in the RevokeAI root
directory, ordinarily `~/revokeai` (Linux/Mac), or
`C:\Users\your_name\revokeai` (Windows). For example, to create an
embedding for the "psychedelic" style, you'd place the training images
into the directory
`~revokeai/text-inversion-training-data/psychedelic`.

## **Launching Training Using the Console Front End**

RevokeAI 2.3 and higher comes with a text console-based training front
end. From within the `revoke.sh`/`revoke.bat` Revoke launcher script,
...
Alternatively, you can select option (8) or from the command line, with the RevokeAI virtual environment active,
you can then launch the front end with the command `revokeai-ti --gui`.
...
"psychedelic" (without angle brackets). Within RevokeAI, you will then
...
this field with `~/revokeai/text-inversion-training-data/my-trigger`,
...
`~/revokeai/text-inversion-output/my-trigger`, but you can change the
...
`~/revokeai/text-inversion-output/my-model/`)

At the end of successful training, the system will copy the file
`learned_embeds.bin` into the RevokeAI root directory's `embeddings`
...
embeddings file in
`~/revokeai/embeddings/psychedelic/learned_embeds.bin`

You may now launch RevokeAI and try out a prompt that uses the trigger
...
command line after activating RevokeAI's virtual environment.
...
revokeai-ti \
       ...
       --train_data_dir=/home/lstein/revokeai/training-data/psychedelic \
       --output_dir=/home/lstein/revokeai/text-inversion-training/psychedelic \
       --scale_lr \
       --train_batch_size=8 \
...
copyright (c) 2023, Lincoln Stein and the RevokeAI Development Team
