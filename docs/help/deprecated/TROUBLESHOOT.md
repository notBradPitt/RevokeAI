...
### `revoke.py` crashes with the complaint that it can't find `ldm.simplet2i.py`
...
`revoke.py` dies, complaining of various missing modules, none of which starts
with `ldm`.

**SOLUTION**

From within the `RevokeAI` directory, run `conda env update` This is also
