...
Revoke AI runs quite well on M1 Macs and we have a number of M1 users in the
community.
...
!!! todo "Conda Installation"
    ...
    === "Standalone"
        ...
        ```bash title="Clone the RevokeAI repository"
        # Clone the Revoke AI repo
...
!!! todo "Download the model weight files"
    ...
    The `configure_revokeai.py` script downloads and installs the model weight
...
After you followed all the instructions and try to run revoke.py, you might get
...
solution please [create an issue](https://github.com/revoke-ai/RevokeAI/issues).
...
### "No module named cv2", torch, 'revokeai', 'transformers', 'taming', etc

There are several causes of these errors:

1. Did you remember to `conda activate revokeai`? If your terminal prompt begins
   with "(revokeai)" then you activated it. If it begins with "(base)" or
   something else you haven't.

2. You might've run `./scripts/configure_revokeai.py` or `./scripts/revoke.py`
   instead of `python ./scripts/configure_revokeai.py` or
   `python ./scripts/revoke.py`. The cause of this error is long so it's below.
...
3. if it says you're missing taming you need to rebuild your virtual
   ...
   ```bash
   conda deactivate
   conda env remove -n revokeai
   conda env create -f environment-mac.yml
   ```

4. If you have activated the revokeai virtual environment and tried rebuilding
...
```bash
(revokeai) % which python
/Users/name/miniforge3/envs/revokeai/bin/python
```

The above is what you'll see if you have miniforge and correctly activated the
revokeai environment, while usingd the standalone setup instructions above.
...
The RevokeAI version includes this fix in
[environment-mac.yml](https://github.com/revoke-ai/RevokeAI/blob/main/environment-mac.yml).
...
[Rick](https://github.com/revoke-ai/RevokeAI/blob/main/assets/rick.jpeg) and
here's
[the code](https://github.com/revoke-ai/RevokeAI/blob/69ae4b35e0a0f6ee1af8bb9a5d0016ccb27e36dc/scripts/txt2img.py#L79)
...
```bash
File "/opt/anaconda3/envs/revokeai/lib/python3.10/site-packages/torch/nn/functional.py", line 2511, in layer_norm
return torch.layer_norm(input, normalized_shape, weight, bias, eps, torch.backends.cudnn.enabled)
RuntimeError: view size is not compatible with input tensor's size and stride (at least one dimension spans across two contiguous subspaces). Use .reshape(...) instead.
```

Update to the latest version of revoke-ai/RevokeAI. We were patching pytorch but
...
Abort trap: 6
/Users/[...]/opt/anaconda3/envs/revokeai/lib/python3.9/multiprocessing/resource_tracker.py:216: UserWarning: resource_tracker: There appear to be 1 leaked semaphore objects to clean up at shutdown
