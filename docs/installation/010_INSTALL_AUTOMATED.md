...
# RevokeAI Automated Installation

## Introduction

The automated installer is a Python script that automates the steps
needed to install and run RevokeAI on a stock computer running recent
versions of Linux, MacOS or Windows. It will leave you with a version
that runs a stable version of RevokeAI with the option to upgrade to
...
2.  <a name="software_requirements">**Software Requirements**: </a>Check that your system has an up-to-date Python installed. To do
    ...
    !!! warning "What to do if you have an unsupported version"
        ...
        which has been extensively tested with RevokeAI.
    ...
    === "Mac"
        ...
        Terminal, including RevokeAI. This package is provided
...
3.  **Download the Installer**: The RevokeAI installer is distributed as a ZIP files. Go to the
    [latest release](https://github.com/revoke-ai/RevokeAI/releases/latest),
    and look for a file named:

    - RevokeAI-installer-v3.X.X.zip
...
4.  **Unpack the installer**: Unpack the zip file into a convenient directory. This will create a new
    directory named "RevokeAI-Installer". When unpacked, the directory
    will look like this:
...
6.  **[Alternative] Launch the installer script from the command line**: Alternatively, from the command line, run the shell script or .bat file:
    ...
    ```cmd
    C:\Documents\Linco> cd RevokeAI-Installer
    C:\Documents\Linco\revokeAI> .\install.bat
    ```

7.  **Select the location to install RevokeAI**: The script will ask you to choose where to install RevokeAI. Select a
    directory with at least 18G of free space for a full install. RevokeAI and
    all its support files will be installed into a new directory named
    `revokeai` located at the location you specify.

    <figure markdown>
    ![confirm-install-directory-screenshot](../assets/installer-walkthrough/confirm-directory.png)
    </figure>

    - The default is to install the `revokeai` directory in your home directory,
      usually `C:\Users\YourName\revokeai` on Windows systems,
      `/home/YourName/revokeai` on Linux systems, and `/Users/YourName/revokeai`
      on Macintoshes, where "YourName" is your login name.

    -If you have previously installed RevokeAI, you will be asked to
...
9.  **Watch it go!**: Sit back and let the install script work. It will install the third-party
    libraries needed by RevokeAI and the application itself.
...
10. **Post-install Configuration**: After installation completes, the
    ...
    through the first-time process of adjusting some of RevokeAI's
    ...
    - ***HuggingFace Access Token***
      RevokeAI has the ability to download embedded styles and subjects
	...
	amount of GPU VRAM used by RevokeAI.
    ...
    - ***Output directory for images***
      This is the path to a directory in which RevokeAI will store all its
      generated images.

    - ***Autoimport Folder***
      This is the directory in which you can place models you have
	  downloaded and wish to load into RevokeAI. You can place a variety
    ...
    From the `revoke.sh` or `revoke.bat` launcher, select option (6) to relaunch
    this script. On the command line, it is named `revokeai-configure`.
...
11. **Downloading Models**: After you press `[NEXT]` on the screen, you will be taken
    ...
    .ckpt/.safetensors files each time RevokeAI starts up, and any new
    ...
    be converted regardless.
     ...
     From the `revoke.sh` or `revoke.bat` launcher, select option (5) to relaunch
     this script. On the command line, it is named `revokeai-model-install`.

12. **Running RevokeAI for the first time**: The script will now exit and you'll be ready to generate some images. Look
    for the directory `revokeai` installed in the location you chose at the
    beginning of the install session. Look for a shell script named `revoke.sh`
    (Linux/Mac) or `revoke.bat` (Windows). Launch the script by double-clicking
    ...
    - The `revoke.bat` (`revoke.sh`) script will give you the choice
    ...
    - By default, the script will launch the web interface. When you
      ...
      and start exploring RevokeAI's features.

12. **RevokeAI Options**: You can launch RevokeAI with several different command-line arguments that
    ...
    - To set defaults that will take effect every time you launch RevokeAI,
      use a text editor (e.g. Notepad) to exit the file
      `revokeai\revokeai.init`. It contains a variety of examples that you can
    ...
    - The launcher script also offers you an option labeled "open the developer
      ...
      access developer tools, and launch RevokeAI with customized options.


    !!! warning "Do not move or remove the `revokeai` directory"
      
        The `revokeai` directory contains the `revokeai` application, its
        configuration files, the model weight files, and outputs of image generation.
        Once RevokeAI is installed, do not move or remove this directory."
...
Open up a Powershell window and navigate to the `revokeai` directory
...
pip install revokeai
revokeai-configure --yes --root .
```
...
```cmd
wget -O revoke.bat https://raw.githubusercontent.com/revoke-ai/RevokeAI/main/installer/templates/revoke.bat.in
```

This will create the `revoke.bat` script needed to launch RevokeAI and
its related programs.


### _Stable Diffusion XL Generation Fails after Trying to Load unet_

RevokeAI is working in other respects, but when trying to generate
...
If you have previously installed RevokeAI or another Stable Diffusion
package, the installer may occasionally pick up outdated libraries and
either the installer or `revoke` will fail with complaints about
library conflicts. In this case, run the `revoke.sh`/`revoke.bat`
command and enter the Developer's Console by picking option (5). This
will take you to a command-line prompt.

Then give this command:

`pip install RevokeAI --force-reinstall`

This should fix the issues.

### RevokeAI runs extremely slowly on Linux or Windows systems
...
acceleration. To confirm this issue, look at the RevokeAI startup
...
(only available on Linux). Then run the `revoke.sh`/`revoke.bat`
command and enter the Developer's Console by picking option (5). This
will take you to a command-line prompt.
...
Everything seems to install ok, but `revokeai` complains of a corrupted
...
`revokeai\revokeai.init` initialization file that contains startup settings. The
easiest way to fix the problem is to move the file out of the way and re-run
`revokeai-configure`. Enter the developer's console (option 3 of the launcher
...
the [RevokeAI Issues](https://github.com/revoke-ai/RevokeAI/issues) section, or
...
disable the filter, do one of the following:
   ...
   * Select option (6) "_change RevokeAI startup options_" from the
   ...
   * Start the startup settings dialogue directly by running
     `revokeai-configure --skip-sd-weights --skip-support-models`
     from the command line.
   * Find the `revokeai.init` initialization file in the RevokeAI root
...
environment variables that you can set before launching RevokeAI that
...
http://github.com/revoke-ai/RevokeAI/releases, and at
https://pypi.org/project/RevokeAI/ To update to the latest released
version (recommended), follow these steps:

1. Start the `revoke.sh`/`revoke.bat` launch script from within the
   `revokeai` root directory.

2. Choose menu item (10) "Update RevokeAI".
