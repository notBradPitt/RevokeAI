...
# This import must happen before other revoke imports or test in other files(!!) break
...
from revokeai.app.invocations.baseinvocation import BaseInvocation, BaseInvocationOutput, InvocationContext
from revokeai.app.invocations.collections import RangeInvocation
from revokeai.app.invocations.math import AddInvocation, MultiplyInvocation
from revokeai.app.services.graph import CollectInvocation, Graph, GraphExecutionState, IterateInvocation, LibraryGraph
...
from .test_revoker import create_edge
...
def revoke_next(g: GraphExecutionState, services: InvocationServices) -> tuple[BaseInvocation, BaseInvocationOutput]:
    n = g.next()
    if n is None:
        return (None, None)

    print(f"invoking {n.id}: {type(n)}")
    o = n.revoke(InvocationContext(services, "1"))
    g.complete(n.id, o)
...
def test_graph_state_executes_in_order(simple_graph, mock_services):
    ...
    n1 = revoke_next(g, mock_services)
    n2 = revoke_next(g, mock_services)
    n3 = g.next()
...
def test_graph_state_collects(mock_services):
    ...
    _ = revoke_next(g, mock_services)
    _ = revoke_next(g, mock_services)
    n6 = revoke_next(g, mock_services)
