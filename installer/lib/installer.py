...
BOOTSTRAP_VENV_PREFIX = "revokeai-installer-tmp"
...
class Installer:
    """
    Deploys an RevokeAI installation into a given path
    """
    ...
    def app_venv(self, path: str = None):
        """
        Create a virtualenv for the RevokeAI installation
        """
        ...
        # experimental / testing
            ...
            venv_dir = Path(venv_dir_parent).expanduser().resolve() / f"RevokeAI/{VERSION}/venv"
        ...
        # so that updates to system python don't break RevokeAI
        try:
    ...
    def install(
        self, root: str = "~/revokeai", version: str = "latest", yes_to_all=False, find_links: Path = None
    ) -> None:
        """
        Install the RevokeAI application into the given runtime path

        :param root: Destination path for the installation
        :type root: str
        :param version: RevokeAI version to install
        :type version: str
        ...
        default_path = os.environ.get("REVOKEAI_ROOT") or Path(root).expanduser().resolve()
        self.dest = default_path if yes_to_all else messages.dest_path(root)

        # create the venv for the app
        self.venv = self.app_venv()

        self.instance = RevokeAiInstance(runtime=self.dest, venv=self.venv, version=version)

        # install dependencies and the RevokeAI application
...
class RevokeAiInstance:
    """
    Manages an installed instance of RevokeAI, comprising a virtual environment and a runtime directory.
    ...
    def __init__(self, runtime: Path, venv: Path, version: str) -> None:
        ...
        set_sys_path(venv)
        os.environ["REVOKEAI_ROOT"] = str(self.runtime.expanduser().resolve())
    ...
    def install(self, extra_index_url=None, optional_modules=None, find_links=None):
        ...
        messages.simple_banner("Installing the RevokeAI Application :art:")
    ...
    def install_app(self, extra_index_url=None, optional_modules=None, find_links=None):
        ...
        if FF_USE_LOCAL_WHEEL:
            ...
            src = f"revokeai=={version}" if version is not None else "revokeai"
    ...
    def configure(self):
        """
        Configure the RevokeAI runtime directory
        """
        ...
        from revokeai.frontend.install.revokeai_configure import revokeai_configure
        ...
        try:
            ...
            if not succeeded:
                print('To try again, find the "revokeai" directory, run the script "revoke.sh" or "revoke.bat"')
