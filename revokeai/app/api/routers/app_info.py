...
from revokeai.app.invocations.upscale import ESRGAN_MODELS
from revokeai.backend.image_util.invisible_watermark import InvisibleWatermark
from revokeai.backend.image_util.patchmatch import PatchMatch
from revokeai.backend.image_util.safety_checker import SafetyChecker
from revokeai.backend.util.logging import logging
...
async def get_log_level() -> LogLevel:
    ...
    return LogLevel(ApiDependencies.revoker.services.logger.level)
...
async def set_log_level(
    ...
    ApiDependencies.revoker.services.logger.setLevel(level)
    return LogLevel(ApiDependencies.revoker.services.logger.level)
