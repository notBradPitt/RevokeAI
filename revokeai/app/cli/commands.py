...
class CliContext:
    ...
    def __init__(self, revoker: Revoker, session: GraphExecutionState, parser: argparse.ArgumentParser):
    ...
    def get_session(self):
        self.session = self.revoker.services.graph_execution_manager.get(self.session.id)
        return self.session

    def reset(self):
        self.session = self.revoker.create_execution_state()
        self.graph_nodes = dict()
        self.nodes_added = list()
    ...
    def add_node(self, node: BaseInvocation):
        ...
        self.revoker.services.graph_execution_manager.set(self.session)
...
class DrawGraphCommand(BaseCommand):
    ...
    def run(self, context: CliContext) -> None:
        session: GraphExecutionState = context.revoker.services.graph_execution_manager.get(context.session.id)
