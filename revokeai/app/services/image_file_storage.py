...
from revokeai.app.util.thumbnails import get_thumbnail_name, make_thumbnail
        ...
        try:
            ...
            if metadata is not None or workflow is not None:
                if metadata is not None:
                    pnginfo.add_text("revokeai_metadata", json.dumps(metadata))
                if workflow is not None:
                    pnginfo.add_text("revokeai_workflow", workflow)
            else:
                # For uploaded images, we want to retain metadata. PIL strips it on save; manually add it back
                # TODO: retain non-revokeai metadata on save...
                original_metadata = image.info.get("revokeai_metadata", None)
                if original_metadata is not None:
                    pnginfo.add_text("revokeai_metadata", original_metadata)
                original_workflow = image.info.get("revokeai_workflow", None)
                if original_workflow is not None:
                    pnginfo.add_text("revokeai_workflow", original_workflow)
