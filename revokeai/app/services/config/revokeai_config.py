# Copyright (c) 2023 Lincoln Stein (https://github.com/lstein) and the RevokeAI Development Team

"""Revokeai configuration system.

Arguments and fields are taken from the pydantic definition of the
model.  Defaults can be set by creating a yaml configuration file that
has a top-level key of "RevokeAI" and subheadings for each of the
categories returned by `revokeai --help`. The file looks like this:
...
RevokeAI:
  ...
  Paths:
    ...
    db_dir: databases
    outdir: /home/lstein/revokeai-main/outputs
    use_memory_db: false
  Logging:
    log_handlers:
    - console
...
The default name of the configuration file is `revokeai.yaml`, located
in REVOKEAI_ROOT. You can replace supersede this by providing any
...
RevokeAIAppConfig.parse_args() will parse the contents of `sys.argv`
...
you call parse_args() it may be overwritten.
 ...
 conf = RevokeAIAppConfig(log_tokenization=True)
...
creation time:
 ...
 conf = RevokeAIAppConfig.get_config(log_tokenization=True)
 conf.parse_args(argv=['--no-log_tokenization'])
 conf.log_tokenization
 # True

Any setting can be overwritten by setting an environment variable of
form: "REVOKEAI_<setting>", as in:

  export REVOKEAI_port=8080
...
The RevokeAIAppConfig object has a series of properties that
resolve paths relative to the runtime root directory. They each return
a Path object:

 root_path          - path to RevokeAI root
...
In most cases, you will want to create a single RevokeAIAppConfig
object for the entire application. The RevokeAIAppConfig.get_config() function
does this:
...
`RevokeAISettings` class and define a Literal field named "type",
which is set to the desired top-level name.  For example, to create a
"RevokeBatch" configuration, define like this:

  class RevokeBatch(RevokeAISettings):
     type: Literal["RevokeBatch"] = "RevokeBatch"
...
This will now read and write from the "RevokeBatch" section of the
config file, look for environment variables named REVOKEBATCH_*, and
...
two configs are kept in separate sections of the config file:
  ...
  RevokeAI:
     Paths:
        root: /home/lstein/revokeai-main
        conf_path: configs/models.yaml
        legacy_conf_dir: configs/stable-diffusion
        outdir: outputs
     ...
...
from .base import RevokeAISettings

INIT_FILE = Path("revokeai.yaml")
DB_FILE = Path("revokeai.db")
LEGACY_INIT_FILE = Path("revokeai.init")
DEFAULT_MAX_VRAM = 0.5


class RevokeAIAppConfig(RevokeAISettings):
    """
    Generate images using Stable Diffusion. Use "revokeai" to launch
    the command-line client (recommended for experts only), or
    "revokeai-web" to launch the web server. Global options
    can be changed by editing the file "REVOKEAI_ROOT/revokeai.yaml" or by
    setting environment variables REVOKEAI_<setting>.
    """

    singleton_config: ClassVar[RevokeAIAppConfig] = None
    singleton_init: ClassVar[Dict] = None

    # fmt: off
    type: Literal["RevokeAI"] = "RevokeAI"
    ...
    # PATHS
    root                : Path = Field(default=None, description='RevokeAI runtime root directory', category='Paths')
    ...
    db_dir              : Path = Field(default='databases', description='Path to RevokeAI databases directory', category='Paths')
    ...
    version             : bool = Field(default=False, description="Show RevokeAI version and exit", category="Other")
    ...
    # See RevokeAIAppConfig subclass below for CACHE and DEVICE categories
    # fmt: on
    ...
    def parse_args(self, argv: List[str] = None, conf: DictConfig = None, clobber=False):
        ...
        if conf is None:
            try:
                ...
                pass
        RevokeAISettings.initconf = conf
    ...
    @classmethod
    def get_config(cls, **kwargs) -> RevokeAIAppConfig:
        """
        This returns a singleton RevokeAIAppConfig configuration object.
        """
        if (
...
def get_revokeai_config(**kwargs) -> RevokeAIAppConfig:
    """
    Legacy function which returns RevokeAIAppConfig.get_config()
    """
    return RevokeAIAppConfig.get_config(**kwargs)


def _find_root() -> Path:
    venv = Path(os.environ.get("VIRTUAL_ENV") or ".")
    if os.environ.get("REVOKEAI_ROOT"):
        root = Path(os.environ["REVOKEAI_ROOT"])
    elif any([(venv.parent / x).exists() for x in [INIT_FILE, LEGACY_INIT_FILE]]):
        root = (venv.parent).resolve()
    else:
        root = Path("~/revokeai").expanduser().resolve()
    return root
