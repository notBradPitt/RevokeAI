...
from .services.config import RevokeAIAppConfig
...
if True:  # hack to make flake8 happy with imports coming after setting up the config
    ...
    import revokeai.backend.util.hotfixes  # noqa: F401 (monkeypatching on import)
    from revokeai.app.services.board_image_record_storage import SqliteBoardImageRecordStorage
    from revokeai.app.services.board_images import BoardImagesService, BoardImagesServiceDependencies
    from revokeai.app.services.board_record_storage import SqliteBoardRecordStorage
    from revokeai.app.services.boards import BoardService, BoardServiceDependencies
    from revokeai.app.services.image_record_storage import SqliteImageRecordStorage
    from revokeai.app.services.images import ImageService, ImageServiceDependencies
    from revokeai.app.services.invocation_stats import InvocationStatsService
    from revokeai.app.services.resource_name import SimpleNameService
    from revokeai.app.services.urls import LocalUrlService
    from revokeai.backend.util.logging import RevokeAILogger
    from revokeai.version.revokeai_version import __version__
    ...
    from .services.revoker import Revoker
    ...
    if torch.backends.mps.is_available():
        import revokeai.backend.util.mps_fixes  # noqa: F401 (monkeypatching on import)
...
def get_node_inputs(invocation: BaseInvocation, context: CliContext) -> dict[str, NodeField]:
    ...
    if node_type is not GraphInvocation:
        ...
        graph: LibraryGraph = context.revoker.services.graph_library.get(context.graph_nodes[invocation.id])
...
def revoke_all(context: CliContext):
    """Runs all invocations in the specified session"""
    context.revoker.revoke(context.session, revoke_all=True)
    ...
    if context.session.has_error():
        for n in context.session.errors:
            context.revoker.services.logger.error(
...
def revoke_cli():
    logger.info(f"RevokeAI version {__version__}")
    ...
    logger.info(f'RevokeAI database location is "{db_location}"')
    ...
    revoker = Revoker(services)
    session: GraphExecutionState = revoker.create_execution_state()
    ...
    context = CliContext(revoker, session, parser)
    set_autocompleter(services)
        ...
        try:
            ...
            for cmd in cmds:
                ...
                if isinstance(command.command, BaseCommand):
                    # Revoke all current nodes to preserve operation order
                    revoke_all(context)
            ...
            revoker.services.logger.warning('Invalid command, use "help" to list commands')
            continue

        except ValidationError:
            revoker.services.logger.warning('Invalid command arguments, run "<command> --help" for summary')

        except SessionError:
            # Start a new session
            revoker.services.logger.warning("Session error: creating a new session")
            context.reset()
...
if __name__ == "__main__":
    if config.version:
        print(f"RevokeAI version {__version__}")
    else:
        revoke_cli()
