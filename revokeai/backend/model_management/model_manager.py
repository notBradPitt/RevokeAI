"""This module manages the RevokeAI `models.yaml` file, mapping
symbolic diffusers model names to the paths and repo_ids used by the
underlying `from_pretrained()` call.

SYNOPSIS:

 mgr = ModelManager('/home/phi/revokeai/configs/models.yaml')
...
providing information about a model defined in models.yaml. For example:
   ...
   >>> json.dumps(models[0])
  {"path": "/home/lstein/revokeai-main/models/sd-1/controlnet/canny",
...
defined in `revokeai.yaml`) will also be scanned for new models at
...
the different types in this way. This entry in `revokeai.yaml` will
...
`revokeai/backend/model_management/models` for the attributes required
by each model type.
...
the root is the RevokeAI ROOTDIR.
...
from revokeai.backend.util import CUDA_DEVICE, Chdir
...
class ModelManager(object):
    ...
    def __init__(
        ...
        self.app_config = RevokeAIAppConfig.get_config()
        self.logger = logger
    ...
    def del_model(
        ...
        # if model inside revoke models folder - delete files
    ...
    def scan_autoimport_directory(self) -> Dict[str, AddModelResult]:
        ...
        from revokeai.backend.install.model_install_backend import ModelInstall
        from revokeai.frontend.install.model_install import ask_user_for_prediction_type
