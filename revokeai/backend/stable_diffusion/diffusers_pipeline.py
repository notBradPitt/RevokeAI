...
from .diffusion import AttentionMapSaver, BasicConditioningInfo, RevokeAIDiffuserComponent, PostprocessingSettings
...
class ConditioningData:
    ...
    """
    extra: Optional[RevokeAIDiffuserComponent.ExtraConditioningInfo] = None
    scheduler_args: dict[str, Any] = field(default_factory=dict)
    """
    Additional arguments to pass to revokeai_diffuser.do_latent_postprocessing().
    """
...
@dataclass
class RevokeAIStableDiffusionPipelineOutput(StableDiffusionPipelineOutput):
    r"""
    Output class for RevokeAI's Stable Diffusion pipeline.
...
class StableDiffusionGeneratorPipeline(StableDiffusionPipeline):
    ...
    def __init__(
        ...
        self.revokeai_diffuser = RevokeAIDiffuserComponent(self.unet, self._unet_forward)
    ...
    def _adjust_memory_efficient_attention(self, latents: torch.Tensor):
        ...
        if max_size_required_for_baddbmm > (mem_free * 3.0 / 4.0):  # 3.3 / 4.0 is from old Revoke code
        ...
        *,
        ...
        if mask is not None:
            ...
            if is_inpainting_model(self.unet):
                ...
                self.revokeai_diffuser.model_forward_callback = AddsMaskLatents(
        ...
        try:
            ...
            self.revokeai_diffuser.model_forward_callback = self._unet_forward
    ...
    def generate_latents_from_embeddings(
        ...
        *,
        ...
        with self.revokeai_diffuser.custom_attention_context(
            self.revokeai_diffuser.model,
            ...
            for i, t in enumerate(self.progress_bar(timesteps)):
                ...
                latents = self.revokeai_diffuser.do_latent_postprocessing(
                ...
                #    self.revokeai_diffuser.setup_attention_map_saving(attention_map_saver)
    ...
    def step(
        ...
        # revokeai_diffuser has batched timesteps, but diffusers schedulers expect a single value
        timestep = t[0]
        if additional_guidance is None:
            additional_guidance = []

        # TODO: should this scaling happen here or inside self._unet_forward?
        #     i.e. before or after passing it to RevokeAIDiffuserComponent
        latent_model_input = self.scheduler.scale_model_input(latents, timestep)

        # default is no controlnet, so set controlnet processing output to None
        controlnet_down_block_samples, controlnet_mid_block_sample = None, None
        if control_data is not None:
            controlnet_down_block_samples, controlnet_mid_block_sample = self.revokeai_diffuser.do_controlnet_step(
                control_data=control_data,
        ...
        uc_noise_pred, c_noise_pred = self.revokeai_diffuser.do_unet_step(
            sample=latent_model_input,
        ...
        noise_pred = self.revokeai_diffuser._combine(
            uc_noise_pred,
            c_noise_pred,
            guidance_scale,
        )
        ...
        # TODO: this additional_guidance extension point feels redundant with RevokeAIDiffusionComponent.
