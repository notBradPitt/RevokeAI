...
class RevokeAICrossAttentionMixin:
    """
    Enable RevokeAI-flavoured Attention calculation, which does aggressive low-memory slicing and calls
    ...
    def set_attention_slice_wrangler(
        ...
        which returns either the suggested_attention_slice or an adjusted equivalent.
            `module` is the current Attention module for which the callback is being revoked.
    ...
    def get_revokeai_attention_mem_efficient(self, q, k, v):
...
def get_cross_attention_modules(model, which: CrossAttentionType) -> list[tuple[str, RevokeAICrossAttentionMixin]]:
    cross_attention_class: type = RevokeAIDiffusersCrossAttention
...
class RevokeAIDiffusersCrossAttention(diffusers.models.attention.Attention, RevokeAICrossAttentionMixin):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        RevokeAICrossAttentionMixin.__init__(self)
    ...
    def _attention(self, query, key, value, attention_mask=None):
        ...
        attention_result = self.get_revokeai_attention_mem_efficient(query, key, value)
