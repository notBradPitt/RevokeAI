# Copyright (c) 2023 Lincoln D. Stein and The RevokeAI Development Team

"""
revokeai.backend.util.logging

Logging class for RevokeAI that produces console messages

Usage:

from revokeai.backend.util.logging import RevokeAILogger

logger = RevokeAILogger.getLogger(name='RevokeAI') // Initialization
(or)
logger = RevokeAILogger.getLogger(__name__) // To use the filename
logger.configure()
...
Console messages:
    [12-05-2023 20]::[RevokeAI]::CRITICAL --> This is an info message [In Bold Red]
    [12-05-2023 20]::[RevokeAI]::ERROR --> This is an info message [In Red]
    [12-05-2023 20]::[RevokeAI]::WARNING --> This is an info message [In Yellow]
    [12-05-2023 20]::[RevokeAI]::INFO --> This is an info message [In Grey]
    [12-05-2023 20]::[RevokeAI]::DEBUG --> This is an info message [In Grey]

Alternate Method (in this case the logger name will be set to RevokeAI):
import revokeai.backend.util.logging as IAILogger
IAILogger.debug('this is a debugging message')

## Configuration

The default configuration will print to stderr on the console. To add
additional logging handlers, call getLogger with an initialized RevokeAIAppConfig
object:
...
When "console" is provided to `--log_handlers`, messages will be written to the command line window in which RevokeAI was launched. By default, the color formatter will be used unless overridden by `--log_format`.
...
You can set and forget logging options by adding a "Logging" section to `revokeai.yaml`:
...
def debug(msg, *args, **kwargs):
    RevokeAILogger.getLogger().debug(msg, *args, **kwargs)


def info(msg, *args, **kwargs):
    RevokeAILogger.getLogger().info(msg, *args, **kwargs)


def warning(msg, *args, **kwargs):
    RevokeAILogger.getLogger().warning(msg, *args, **kwargs)


def error(msg, *args, **kwargs):
    RevokeAILogger.getLogger().error(msg, *args, **kwargs)


def critical(msg, *args, **kwargs):
    RevokeAILogger.getLogger().critical(msg, *args, **kwargs)


def log(level, msg, *args, **kwargs):
    RevokeAILogger.getLogger().log(level, msg, *args, **kwargs)


def disable(level=logging.CRITICAL):
    RevokeAILogger.getLogger().disable(level)


def basicConfig(**kwargs):
    RevokeAILogger.getLogger().basicConfig(**kwargs)


def getLogger(name: str = None) -> logging.Logger:
    return RevokeAILogger.getLogger(name)
...
class RevokeAIFormatter(logging.Formatter):
    """
...
class RevokeAISyslogFormatter(RevokeAIFormatter):
    """
...
class RevokeAILegacyLogFormatter(RevokeAIFormatter):
    """
    Formatting for the RevokeAI Logger (legacy version)
    """
...
class RevokeAIPlainLogFormatter(RevokeAIFormatter):
    """
    Custom Formatting for the RevokeAI Logger (plain version)
    """
...
class RevokeAIColorLogFormatter(RevokeAIFormatter):
    """
    Custom Formatting for the RevokeAI Logger
    """
...
LOG_FORMATTERS = {
    "plain": RevokeAIPlainLogFormatter,
    "color": RevokeAIColorLogFormatter,
    "syslog": RevokeAISyslogFormatter,
    "legacy": RevokeAILegacyLogFormatter,
}


class RevokeAILogger(object):
    loggers = dict()

    @classmethod
    def getLogger(
        cls, name: str = "RevokeAI", config: RevokeAIAppConfig = RevokeAIAppConfig.get_config()
    ) -> logging.Logger:
    ...
    @classmethod
    def getLoggers(cls, config: RevokeAIAppConfig) -> list[logging.Handler]:
