...
RevokeAI 2.3 installation to 3.0.0.
"""
...
from revokeai.backend.model_management import ModelManager
from revokeai.backend.model_management.model_probe import BaseModelType, ModelProbe, ModelProbeInfo, ModelType
...
def get_legacy_embeddings(root: Path) -> ModelPaths:
    path = root / "revokeai.init"
    if path.exists():
        return _parse_legacy_initfile(root, path)
    path = root / "revokeai.yaml"
    if path.exists():
...
def do_migrate(src_directory: Path, dest_directory: Path):
    """
    Migrate models from src to dest RevokeAI root directories
    """
...
def main():
    parser = argparse.ArgumentParser(
        prog="revokeai-migrate3",
        description="""
This will copy and convert the models directory and the configs/models.yaml from the RevokeAI 2.3 format
'--from-directory' root to the RevokeAI 3.0 '--to-directory' root. These may be abbreviated '--from' and '--to'.a

The old models directory and config file will be renamed 'models.orig' and 'models.yaml.orig' respectively.
It is safe to provide the same directory for both arguments, but it is better to use the revokeai_configure
script, which will perform a full upgrade in place.""",
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help='Source RevokeAI 2.3 root directory (containing "revokeai.init" or "revokeai.yaml")',
    )
    parser.add_argument(
        "--to-directory",
        dest="dest_root",
        type=Path,
        required=True,
        help='Destination RevokeAI 3.0 directory (containing "revokeai.yaml")',
    )
    ...
    assert (src_root / "revokeai.init").exists() or (
        src_root / "revokeai.yaml"
    ).exists(), f"{src_root} does not contain an RevokeAI init file."
    ...
    # TODO: revisit - don't rely on revokeai.yaml to exist yet!
    dest_is_setup = (dest_root / "models/core").exists() and (dest_root / "databases").exists()
    if not dest_is_setup:
        from revokeai.backend.install.revokeai_configure import initialize_rootdir
