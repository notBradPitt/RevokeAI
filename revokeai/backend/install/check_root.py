"""
Check that the revokeai_root is correctly configured and exit if not.
"""
import sys
...
def check_revokeai_root(config: RevokeAIAppConfig):
    try:
        ...
        print("** One or more necessary files is missing from your RevokeAI root directory **")
        print("** Please rerun the configuration script to fix this problem. **")
        print("** From the launcher, selection option [7]. **")
        print(
            '** From the command line, activate the virtual environment and run "revokeai-configure --yes --skip-sd-weights" **'
        )
        print(
