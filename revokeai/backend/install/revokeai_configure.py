...
import revokeai.configs as configs
from revokeai.app.services.config import RevokeAIAppConfig
from revokeai.backend.install.legacy_arg_parsing import legacy_parser
from revokeai.backend.install.model_install_backend import InstallSelections, ModelInstall, hf_download_from_pretrained
from revokeai.backend.model_management.model_probe import BaseModelType, ModelType
from revokeai.backend.util.logging import RevokeAILogger
from revokeai.frontend.install.model_install import addModelsForm, process_and_execute

# TO DO - Move all the frontend code into revokeai.frontend.install
from revokeai.frontend.install.widgets import (
    MIN_COLS,
    MIN_LINES,
    CenteredButtonPress,
    CyclingForm,
...
def get_literal_fields(field) -> list[Any]:
    return get_args(get_type_hints(RevokeAIAppConfig).get(field))


# --------------------------globals-----------------------

config = RevokeAIAppConfig.get_config()
...
INIT_FILE_PREAMBLE = """# RevokeAI initialization file
...
logger = RevokeAILogger.getLogger()
...
If you installed using an installation script, run:
  {config.root_path}/revoke.{"bat" if sys.platform == "win32" else "sh"}
...
def get_root(root: str = None) -> str:
    if root:
        return root
    elif os.environ.get("REVOKEAI_ROOT"):
        return os.environ.get("REVOKEAI_ROOT")
    else:
...
class editOptsForm(CyclingForm, npyscreen.FormMultiPage):
    ...
    def create(self):
        ...
        old_opts = self.parentApp.revokeai_opts
        first_time = not (config.root_path / "revokeai.yaml").exists()
...
class EditOptApplication(npyscreen.NPSAppManaged):
    def __init__(self, program_opts: Namespace, revokeai_opts: Namespace):
        super().__init__()
        self.program_opts = program_opts
        self.revokeai_opts = revokeai_opts
...
def edit_opts(program_opts: Namespace, revokeai_opts: Namespace) -> argparse.Namespace:
    editApp = EditOptApplication(program_opts, revokeai_opts)
    editApp.run()
    return editApp.new_opts()


def default_startup_options(init_file: Path) -> Namespace:
    opts = RevokeAIAppConfig.get_config()
    return opts
...
def initialize_rootdir(root: Path, yes_to_all: bool = False):
    logger.info("Initializing RevokeAI runtime directory")
...
def run_console_ui(program_opts: Namespace, initfile: Path = None) -> (Namespace, Namespace):
    revokeai_opts = default_startup_options(initfile)
    revokeai_opts.root = program_opts.root
...
def write_opts(opts: Namespace, init_file: Path):
    """
    Update the revokeai.yaml file with values from current settings.
    """
    # this will load current settings
    new_config = RevokeAIAppConfig.get_config()
...
def migrate_init_file(legacy_format: Path):
    ...
    new = RevokeAIAppConfig.get_config()

    fields = [x for x, y in RevokeAIAppConfig.__fields__.items() if y.field_info.extra.get("category") != "DEPRECATED"]
    for attr in fields:
        if hasattr(old, attr):
            try:
    ...
    revokeai_yaml = legacy_format.parent / "revokeai.yaml"
    with open(revokeai_yaml, "w", encoding="utf-8") as outfile:
        outfile.write(new.to_yaml())

    legacy_format.replace(legacy_format.parent / "revokeai.init.orig")


# -------------------------------------
def migrate_models(root: Path):
    from revokeai.backend.install.migrate_to_3 import do_migrate
...
def migrate_if_needed(opt: Namespace, root: Path) -> bool:
    ...
    old_init_file = root / "revokeai.init"
    new_init_file = root / "revokeai.yaml"
    ...
    if migration_needed:
        if opt.yes_to_all or yes_or_no(
            ...
            logger.info("** Migrating revokeai.init to revokeai.yaml")
...
# -------------------------------------
def main():
    parser = argparse.ArgumentParser(description="RevokeAI model downloader")
    parser.add_argument(
        "--skip-sd-weights",
    ...
    revoke_args = []
    if opt.root:
        revoke_args.extend(["--root", opt.root])
    if opt.full_precision:
        revoke_args.extend(["--precision", "float32"])
    config.parse_args(revoke_args)
    logger = RevokeAILogger().getLogger(config=config)
    ...
    try:
        ...
        new_init_file = config.root_path / "revokeai.yaml"
        ...
        if opt.yes_to_all:
            ...
            if init_options:
                ...
                logger.info('\n** CANCELLED AT USER\'S REQUEST. USE THE "revoke.sh" LAUNCHER TO RUN LATER **\n')
                sys.exit(0)
