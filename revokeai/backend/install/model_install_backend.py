...
from revokeai.backend.model_management import AddModelResult, BaseModelType, ModelManager, ModelType, ModelVariantType
from revokeai.backend.model_management.model_probe import ModelProbe, ModelProbeInfo, SchedulerPredictionType
from revokeai.backend.util import download_with_resume
from revokeai.backend.util.devices import choose_torch_device, torch_dtype

from ..util.logging import RevokeAILogger

warnings.filterwarnings("ignore")

# --------------------------globals-----------------------
config = RevokeAIAppConfig.get_config()
logger = RevokeAILogger.getLogger(name="RevokeAI")

# the initial "configs" dir is now bundled in the `revokeai.configs` package
...
def hf_download_from_pretrained(model_class: object, model_name: str, destination: Path, **kwargs):
    logger = RevokeAILogger.getLogger("RevokeAI")
