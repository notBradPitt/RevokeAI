"""
revokeai.frontend.merge exports a single function call merge_diffusion_models()
used to merge 2-3 models together and create a new RevokeAI-registered diffusion model.
...
from revokeai.backend.model_management import BaseModelType, ModelManager, ModelMerger, ModelType
from revokeai.frontend.install.widgets import FloatTitleSlider, SingleSelectColumns, TextBox

config = RevokeAIAppConfig.get_config()


def _parse_args() -> Namespace:
    parser = argparse.ArgumentParser(description="RevokeAI model merging")
    parser.add_argument(
        "--root_dir",
        type=Path,
        default=config.root,
        help="Path to the revokeai runtime directory",
    )
    parser.add_argument(
        "--front_end",
        "--gui",
