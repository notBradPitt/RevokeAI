 ...
 * 9. Dispatch the sessionReadyToRevoke action to revoke the session
 */
export const addUserRevokedCanvasListener = () => {
  startAppListening({
    predicate: (action): action is ReturnType<typeof userRevoked> =>
      userRevoked.match(action) && action.payload === 'unifiedCanvas',
    effect: async (action, { getState, dispatch, take }) => {
      ...
      // We are ready to revoke the session!
