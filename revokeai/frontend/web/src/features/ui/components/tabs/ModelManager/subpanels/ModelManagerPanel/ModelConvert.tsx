...
type SaveLocation = 'RevokeAIRoot' | 'Custom';
...
export default function ModelConvert(props: ModelConvertProps) {
  ...
  const [saveLocation, setSaveLocation] =
    useState<SaveLocation>('RevokeAIRoot');
  const [customSaveLocation, setCustomSaveLocation] = useState<string>('');

  useEffect(() => {
    setSaveLocation('RevokeAIRoot');
  }, [model]);
    ...
    >
      ...
      <Flex flexDir="column" gap={2}>
        <Flex marginTop={4} flexDir="column" gap={2}>
          ...
          >
            <Flex gap={4}>
              <Radio value="RevokeAIRoot">
                <Tooltip label="Save converted model in the RevokeAI root folder">
                  {t('modelManager.revokeRoot')}
                </Tooltip>
              </Radio>
              <Radio value="Custom">
