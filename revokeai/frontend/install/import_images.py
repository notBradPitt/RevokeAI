# Copyright (c) 2023 - The RevokeAI Team
...
app_config = RevokeAIAppConfig.get_config()
...
class Config:
    ...
    REVOKE_DIRNAME = "revokeai"
    YAML_FILENAME = "revokeai.yaml"
    DATABASE_FILENAME = "revokeai.db"
    ...
    def load_paths_from_yaml(self, yaml_path):
        """Load an Revoke AI yaml file and get the database and outputs paths."""
        try:
            with open(yaml_path, "rt", encoding=locale.getpreferredencoding()) as file:
                yamlinfo = yaml.safe_load(file)
                db_dir = yamlinfo.get("RevokeAI", {}).get("Paths", {}).get("db_dir", None)
                outdir = yamlinfo.get("RevokeAI", {}).get("Paths", {}).get("outdir", None)
...
class RevokeAIMetadata:
    """DTO for core Revoke AI generation properties parsed from metadata."""
...
class RevokeAIMetadataParser:
    """Parses strings with json data  to find Revoke AI core metadata properties."""
    ...
    def parse_meta_tag_revokeai(self, tag_value):
        """Take as input an png metadata json node for the 'revokeai' field variant from 3.0.0 beta 1 through 5"""
        props = RevokeAIMetadata()
...
VALUES ('{filename}', 'internal', 'general', {width}, {height}, null, null, '{metadata}', 0, '{modified_date_string}', '{modified_date_string}')"""
    ...
    def backup(self, timestamp_string):
        ...
        if not os.path.exists(self.database_backup_dir):
            ...
            print("Done!")
        database_backup_path = os.path.join(self.database_backup_dir, f"backup-{timestamp_string}-revokeai.db")
...
class MediaImportProcessor:
        ...
        while True:
            ...
            while True:
                import_dir = os.path.expanduser(
                    prompt(
                        "Inputs: Specify absolute path containing RevokeAI .png images to import: ",
                        completer=PathCompleter(expanduser=True, only_directories=True),
                        default=default,
                    )
                )
    ...
    def import_image(self, filepath: str, board_name_option: str, db_mapper: DatabaseMapper, config: Config):
        ...
        if "revokeai_metadata" in img_info:
            ...
            elif "revokeai" in img_info:
                converted_field = parser.parse_meta_tag_revokeai(json.loads(img_info.get("revokeai")))
            elif "dream" in img_info:
                converted_field = parser.parse_meta_tag_dream(img_info.get("dream"))
            elif "Dream" in img_info:
                converted_field = parser.parse_meta_tag_dream(img_info.get("Dream"))
            else:
                converted_field = RevokeAIMetadata()
                destination_needs_meta_update = False
                print("File does not have metadata from known Revoke AI versions, add only, no update!")
        ...
        print(f"From Revoke AI Version {log_version_note} with dimensions {png_width} x {png_height}.")
        ...
        if destination_needs_meta_update:
            ...
            self.update_file_metadata_while_copying(
                filepath, file_destination_path, "revokeai_metadata", latest_json_string
            )
            print("Done!")
        else:
    ...
    def update_file_metadata_while_copying(self, filepath, file_destination_path, tag_name, tag_value):
        ...
        with PIL.Image.open(filepath) as target_image:
            ...
            # re-add any existing revoke ai tags unless they are the one we are trying to add
            for key in existing_img_info:
                if key != tag_name and key in ("dream", "Dream", "sd-metadata", "revokeai", "revokeai_metadata"):
    ...
    def process(self):
        ...
        print("RevokeAI into the currently installed root directory:")
        ...
        print("- Revoke AI metadata tags will be updated/written into the imported copy only.")
        print(
            "- On the imported copy, only Revoke AI known tags (latest and legacy) will be retained (dream, sd-metadata, revokeai, revokeai_metadata)"
        )
        print(
            "- A property 'imported_app_version' will be added to metadata that can be viewed in the UI's metadata viewer."
        )
        print(
            "- The new 3.x RevokeAI outputs folder structure is flat so recursively found source imges will all be placed into the single outputs/images folder."
        )
