...
from revokeai.backend.install.model_install_backend import InstallSelections, ModelInstall, SchedulerPredictionType
from revokeai.backend.model_management import ModelManager, ModelType
from revokeai.backend.util import choose_precision, choose_torch_device
...
class addModelsForm(CyclingForm, npyscreen.FormMultiPage):
    ...
    def create(self):
        ...
        if not config.model_conf_path.exists():
            with open(config.model_conf_path, "w") as file:
                print("# RevokeAI model configuration file", file=file)
    ...
    def add_pipeline_widgets(
        ...
        **kwargs,
        ...
        widgets = self.add_model_widgets(
            ...
            window_width=window_width,
            install_prompt=f"Installed {model_type.value.title()} models. Unchecked models in the RevokeAI root directory will be deleted. Enter URLs, paths or repo_ids to import.",
            **kwargs,
        )
    ...
    def marshall_arguments(self):
        ...
        .autoscan_on_startup:  True if revokeai should scan and import at startup time
...
def main():
    ...
    if not config.model_conf_path.exists():
        logger.info("Your RevokeAI root directory is not set up. Calling revokeai-configure.")
